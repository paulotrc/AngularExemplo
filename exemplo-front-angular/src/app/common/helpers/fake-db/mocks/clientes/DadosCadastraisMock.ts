...
export class DadosCadastraisMock {
    public static itens = () => {

            const itens = JSON.parse(localStorage.getItem('dadosCadastraisList') ) || [];

            if ( undefined === localStorage.getItem('dadosCadastraisList') || null === localStorage.getItem('dadosCadastraisList')
            || JSON.parse(localStorage.getItem('dadosCadastraisList') )?.length === 0) {
                for (let i = 0; i < 20; i++) {
                ...
                localStorage.setItem('dadosCadastraisList', JSON.stringify(itens));
    ...
    public static item = (id) => {
        const itens = DadosCadastraisMock.itens();
        return itens[id];
    }
}
