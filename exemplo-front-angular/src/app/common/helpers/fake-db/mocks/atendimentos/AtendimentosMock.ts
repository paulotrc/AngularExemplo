...
export class AtendimentosMock {
    ...
    public static itens = () => {
        ...
        if (undefined === localStorage.getItem('atendimentosList') || null === localStorage.getItem('atendimentosList')
            || JSON.parse(localStorage.getItem('atendimentosList'))?.length === 0) {
            ...
            for (let i = 0; i < 20; i++) {
                ...
                if ( AtendimentosMock.arrPilhaEstados.length === 0 ) {
                    AtendimentosMock.arrPilhaEstados = AtendimentosMock.arrEstados.slice();
                }
                const estadoSorteado = AtendimentosMock.arrPilhaEstados.pop();
            ...
            localStorage.setItem('atendimentosList', JSON.stringify(itens));
        }
        return itens;
    };

    public static item = (id) => {
        return JSON.parse(localStorage.getItem('atendimentosList'))[id];
    };
