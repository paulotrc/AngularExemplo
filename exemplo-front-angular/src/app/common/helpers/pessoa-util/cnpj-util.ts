...
export type isValidCnpj = (control: AbstractControl) => Validators;
...
const isValidCnpj: isValidCnpj = (control: AbstractControl): Validators => {

        if ( null === control.value || control.value.trim() === '' ) {
            return null;
        }

        return isValid(control.value, true) ? null : { cnpjNotValid: true } ;
};
